---
description: Rules to finish off and deliver to user set of tasks that have been completed using Super Agent
globs:
alwaysApply: false
version: 1.0
encoding: UTF-8
...
Use the project-manager subagent to read @.super_agent/product/roadmap.md and mark roadmap items as complete with [x] ONLY IF the executed tasks have completed any roadmap item(s) and the spec completes that item.
...
Use the project-manager subagent to create a recap document in .super_agent/recaps/ folder that summarizes what was built for this spec.

<instructions>
  ACTION: Use project-manager subagent
  REQUEST: "Create recap document for current spec:
            - Create file: .super_agent/recaps/[SPEC_FOLDER_NAME].md
...
<recap_template>
  ...
  This recaps what was built for the spec documented at .super_agent/specs/[spec-folder-name]/spec.md.
...
<file_creation>
  <location>.super_agent/recaps/</location>
